...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  if (!tmp) {
    ...
    tmp = strcpy(tmp - options.count, str);
    options.count = 0;
    va_list args;
...
unsigned long atoul(char *str, char **strlim, int base) {
...
int opu(va_list args, ops *op, char **src, int base) {
  ...
  char *strlim;
  check_buffer(src, op);
  if (atoul(*src, NULL, base) ||
      *(*src + strspn(*src, "\n\t \f\x0B\r+")) == '0') {
    if (!op->supr) {
      if (atoul(*src, NULL, base) || **src == '0') res++;
      if (op->len == 1)
        *(short unsigned *)va_arg(args, void *) = atoul(*src, &strlim, base);
      else if (op->len == 2)
        *(long unsigned *)va_arg(args, void *) = atoul(*src, &strlim, base);
      else
        *(unsigned *)va_arg(args, void *) = atoul(*src, &strlim, base);
    } else {
      atoul(*src, &strlim, base);
    }
    op->count += strlim - *src;
    *src = strlim;
  } else {
...
int opp(va_list args, ops *op, char **src) {
  int res = 0;
  char *strlim;
  if (atoul(*src, NULL, 16) ||
      *(*src + strspn(*src, "\n\t \f\x0B\r+-")) == '0') {
    check_buffer(src, op);
    if (!op->supr) {
      if (atoul(*src, NULL, 16) || **src == '0') res++;
      *(void **)va_arg(args, void **) = (void *)atoul(*src, &strlim, 16);
    } else {
      atoul(*src, &strlim, 16);
    }
    op->count += strlim - *src;
...
void check_buffer(char **src, ops *op) {
  oksym(src, op);
  if (!op->buff && op->wid && op->wid < (int)strlen(*src)) {
    op->buff = *(*src + op->wid);
    *(*src + op->wid) = '\0';
  }
}
